SWEP.PrintName = "Computer parts"
SWEP.Purpose = "Computer parts to create main computer."
SWEP.Instructions = "Left mouse button to place main computer."
...
SWEP.VElements = {
	...
	["Left hand thing"] = { type = "Model", model = "models/combine_helicopter/helicopter_bomb01.mdl", bone = "ValveBiped.Bip01_L_Forearm", rel = "", pos = Vector(16.104, 0.518, -1.558), angle = Angle(134.416, 0, 0), size = Vector(0.107, 0.107, 0.107), color = Color(255, 0, 0, 255), surpresslightning = false, material = "", skin = 0, bodygroup = {} }
}
...
function SWEP:PrimaryAttack() 
	if SERVER then
		...
		if self.Owner:GetPos():DistToSqr(eyeTrace.HitPos) < 40000 then
			local entity = ents.Create("territorywars.group_main_computer")
			entity:Spawn()
			entity:SetGroupOwner(self.Owner:TWGetGroup())
			local min, max = entity:GetPhysicsObject():GetAABB()
			local pos = eyeTrace.HitPos
			entity:SetPos(pos)
			self.Owner:TWGetGroup():OnComputerCreated()
			self.Owner:StripWeapon("territorywars.leader_computer_parts")
		end
	end
end
